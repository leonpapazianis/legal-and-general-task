...
describe('FixedAmountOffProductStrategy', () => {
  ...
  it('applies fixed amount off per unit of matching product', () => {
    const discount = makeDiscount(DiscountType.FIXED_AMOUNT_OFF_PRODUCT, {
      productId: 'p1',
      amountOff: 3,
    });
...
describe('BuyXGetYFreeStrategy', () => {
  ...
  it('gives correct free items for one qualifying group', () => {
    ...
    const discount = makeDiscount(DiscountType.BUY_X_GET_Y_FREE, {
      productId: 'p1',
      buyQuantity: 2,
      getFreeQuantity: 1,
    });
...
describe('CartThresholdPercentageStrategy', () => {
  ...
  it('applies percentage discount when subtotal meets threshold', () => {
    const discount = makeDiscount(DiscountType.CART_THRESHOLD_PERCENTAGE, {
      thresholdAmount: 100,
      percentage: 10,
    });
