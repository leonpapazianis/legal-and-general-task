...
import { IsString, IsNotEmpty, IsEnum, IsBoolean, IsOptional, IsObject, IsNumber, IsPositive, IsInt, Min } from 'class-validator';
...
class PercentageOffProductConfigDto {
  ...
  @IsNumber() @IsPositive() @Min(1) percentage: number;
}
...
class FixedAmountOffProductConfigDto {
  ...
  @IsNumber() @IsPositive() amountOff: number;
}
...
class BuyXGetYFreeConfigDto {
  ...
  @IsInt() @Min(1) buyQuantity: number;
  @ApiProperty({ example: 1 })
  @IsInt() @Min(1) getFreeQuantity: number;
}

class CartThresholdPercentageConfigDto {
  @ApiProperty({ example: 100 })
  @IsNumber() @IsPositive() thresholdAmount: number;
